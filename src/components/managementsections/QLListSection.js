...
export class QLListSection extends Component {
    ...
    submitUpdate = async () => {
        ...
        formData.append('file', this.state.file);
        try {
            const result = await sectionApi.update(this.state.selectedSection, formData);
            if (result) {
                if (result.status === 200) {
    ...
    modalQuizzesEdit = () => {
        this.state.modalQuizzesEdit ? this.setState({modalQuizzesEdit: false}): this.setState({modalQuizzesEdit: true})
    }
    render() {
        const renderSection = this.state.sections.map((section) =>
            <tr key={section.id}>
                <td style={{width: "250px"}}>{section.sectionName}</td>
                <td style={{width: "60px"}}><img src={section.photoUrl} alt="" width="50px" height="50px" /></td>
                <td style={{width: "300px"}}>{section.description}</td>
                <td style={{width: "50px"}}>{section.totalQuizzes}</td>
                <td style={{width: "80px"}}><Switch id={section.id} onChange={(props, event, id) => this.requireLogin(id)} checked={section.requireLogin}></Switch></td>
                <td>
                    <Button data-id={section.id} variant="primary" className="btn btn-primary mr-2" onClick={e => { this.modalEdit(e); this.selectSection(e) }}><i data-id={section.id} className="fa fa-edit" /></Button>
                    <Button data-id={section.id} variant="primary" className="btn btn-primary mr-2" onClick={e => { this.modalQuizzesEdit(e); this.selectSection(e) }}><i data-id={section.id} className="fa fa-bars" /></Button>
                    <Button data-id={section.id} variant="primary" className="btn btn-danger" onClick={e => { this.modalDelete(e); this.selectSection(e) }}><i data-id={section.id} className="fa fa-trash" /></Button>
                </td>
            </tr>
        );
        ...
        return (
            <div>
                ...
                <Modal show={this.state.modalCreate} onHide={this.modalCreate}>
                    ...
                    <Modal.Body>
                        <form id="createForm" onSubmit={this.submitCreate}>
                ...
                <Modal show={this.state.modalEdit} onHide={this.modalEdit}>
                    ...
                    <Modal.Footer>
                        ...
                        <Button variant="primary" onClick={this.submitUpdate}>Lưu lại</Button>
                    </Modal.Footer>
                </Modal>
                <Modal show={this.state.modalQuizzesEdit} onHide={this.modalQuizzesEdit}>
                    <Modal.Header closeButton onClick={() => this.modalQuizzesEdit()}>
                        <Modal.Title>Quản lý quizzes</Modal.Title>
                    </Modal.Header>
                    <Modal.Body>
                        <ModalListQuizzes id={this.state.selectedSection}></ModalListQuizzes>
                    </Modal.Body>
                    <Modal.Footer>
                        <Button variant="secondary" onClick={() => this.modalQuizzesEdit()}>Trở lại</Button>
                        <Button variant="primary" onClick={this.submitUpdate}>Lưu lại</Button>
                    </Modal.Footer>
                </Modal>
